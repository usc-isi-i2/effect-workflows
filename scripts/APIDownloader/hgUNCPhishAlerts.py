...
if __name__ == "__main__":
    ...
    if(args.date == "1970-01-01T00:00:00Z"):
        url_cve = "https://effect.hyperiongray.com/api/unc-phish-alerts" #To get everything
    else:
        url_cve = "https://effect.hyperiongray.com/api/unc-phish-alerts/updates/" + str(args.date)
    ...
    while page_num < total_pages:
        ...
        if results_json is not None and "results" in results_json:
            ...
            print "Downloaded ", num_results, " new unc-phish-alerts data rows. Adding them to CDR. Page:", (page_num+1), " of ", total_pages
            if num_results > 0:
                apiDownloader.load_into_cdr(results, "hg-unc-phish-alerts", args.team, "hg-unc-phish-alerts")
                print "Done loading into CDR"
                print "Taking backup on S3"

                rdd = sc.parallelize(results)
                rdd.map(lambda x: ("hg-unc-phish-alerts", json.dumps(x))).saveAsSequenceFile(args.outputFolder + "/hg-unc-phish-alerts/" + str(page_num))
